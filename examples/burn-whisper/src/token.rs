...
pub fn find_language(s: &str) -> Option<Language> {
    Language::iter().find(|l| l.as_str() == s)
}
...
impl Gpt2Tokenizer {
    pub fn new(path: impl Into<PathBuf>) -> Result<Self> {
        let tokenizer = tokenizers::Tokenizer::from_file(path.into())?;
    ...
    pub fn decode(&self, tokens: &[usize], skip_special: bool) -> Result<String> {
        let tokens = tokens.iter().map(|t| *t as u32).collect::<Vec<_>>();
